<?php
	...
	if( isset($_POST['courselogin']) ) {
		require_once( "../connectVar.php" );
		$username = mysqli_real_escape_string( $conn, trim($_POST['username']) );
		$pw = sha1( mysqli_real_escape_string( $conn, trim($_POST['pwd']) ) );
		...
		if( mysqli_num_rows($result) == 0 ) {
			// NO such account.
			echo '<script type="text/javascript">alert("資料錯誤！");location.href="index.php"</script>';
		} else {
